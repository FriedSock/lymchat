(ns lymchat.shared.scene.profile
  (:require [reagent.core :as r]
            ...
            [lymchat.shared.ui :refer [text view image touchable-highlight touchable-opacity button icon icon-button material-icon-button colors gradient scroll alert input dimensions run-after-interactions action-sheet prompt]]
     ...
     (let [current-user (subscribe [:current-user])
       ...
       (let [user (js->clj user :keywordize-keys true)
         ...
         [image {:style {:width width
          ...
          ;; (when-not self?
          ;;  [touchable-opacity {:on-press #(show-action-sheet user in-channel?)
