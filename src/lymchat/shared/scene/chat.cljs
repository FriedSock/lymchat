(ns lymchat.shared.scene.chat
  (:require [reagent.core :as r]
            ...
            [lymchat.shared.ui :refer [text view touchable-highlight list-view button icon image progress moment touchable-opacity colors icon-button dimensions alert
                                       ...
                                       input modalbox modal gradient lym-logo
                                       gifted-chat action-sheet clipboard
                                       material-icon-button parsed-text open-url activity-indicator prompt]]
       ...
       (.showActionSheetWithOptions
        ...
        (fn [i]
          (case i
            ...
            1 (prompt (str "I want to report this picture.")
                      nil
                      (fn [title]
                        (when title
        ...
        (fn [i]
          (case i
            ...
            1 (if channel-id
                (dispatch [:delete-channel-message channel-id message-id])
                (dispatch [:delete-message message-id]))
            2 (prompt (str "I want to report this message.")
                      nil
                      (fn [title]
                        (when title
                          (dispatch [:report {:type "user"
  ...
  (let [channel (js->clj channel :keywordize-keys true)
    ...
    (r/create-class
     {:component-will-mount (fn []
                              (util/show-header)
                              (when (empty? @messages)
                                (dispatch [:load-channel-latest-messages (:id channel)])))
      :reagent-render
      (fn []
        [view {:style {:flex 1}}
