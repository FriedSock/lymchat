(ns lymchat.shared.scene.me
  (:require [reagent.core :as r]
            ...
            [lymchat.shared.ui :refer [text view touchable-highlight button colors gradient material-icon icon-button input alert image switch open-url  scroll touchable-opacity modal dimensions StatusBar activity-indicator prompt] :as ui]
       ...
       [view {:style {:min-height 578}}
         ...
         (when-not (empty? @invites)
           [view {:style {:flex 1
            ...
            [touchable-opacity {:style {:padding-top 10
                                        :padding-left 10}
                                :on-press #(dispatch [:nav/push {:key :invite-request
        ...
        [view {:style (:setting-item styles)}
         [text {:style {:font-size 14}}
          "Don't vibrate"]
         [switch {:on-value-change #(dispatch [:set-no-disturb? %])
        ...
        [view {:style (:setting-item styles)}
         [text {:style {:font-size 14}}
          "Don't invite me"]
         [switch {:on-value-change #(dispatch [:set-no-dm %])
                  :on-tint-color "#65BC54"
                  :value (:no_invite @me)}]]
        ...
        (when (ui/ios?)
          [touchable-highlight
           {:on-press #(dispatch [:jump-in-lym realm/lym])}
           [view (:setting-icon-item styles)
